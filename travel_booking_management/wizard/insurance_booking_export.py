...
class InsuranceBookingExport(models.TransientModel):
    _name = 'travel.insurance.booking.export'
    _description = 'Export Insurance Bookings to Excel'
    ...
    def action_export(self):
        ...
        if self.env.context.get('active_ids') and self.env.context.get('active_model') == 'travel.insurance.booking':
            domain = [('id', 'in', self.env.context['active_ids'])]
            if self.state_filter and self.state_filter != 'all':
                domain.append(('state', '=', self.state_filter))

        bookings = self.env['travel.insurance.booking'].search(domain, order='booking_date asc')
        wb = openpyxl.Workbook()
        ws = wb.active
        ws.title = 'Insurance Bookings'
        ...
        headers = [
            ...
            'Employee Code', 'Doc No/Req By', 'Passenger Name',
        ...
        wb.save(fp)
        fp.seek(0)
        self.write({'excel_file': base64.b64encode(fp.read()), 'file_name': 'insurance_bookings.xlsx'})
        fp.close()
