...
class PackageTourExport(models.TransientModel):
    _name = 'travel.package.tour.booking.export'
    _description = 'Export Package Tour Bookings to Excel'
    ...
    def action_export(self):
        ...
        if self.env.context.get('active_ids') and self.env.context.get('active_model') == 'travel.package.tour.booking':
            domain = [('id', 'in', self.env.context['active_ids'])]
            if self.state_filter and self.state_filter != 'all':
                domain.append(('state', '=', self.state_filter))

        bookings = self.env['travel.package.tour.booking'].search(domain, order='booking_date asc')
        wb = openpyxl.Workbook()
        ws = wb.active
        ws.title = 'Package Tour Bookings'
        ...
        headers = [
            ...
            'Employee Code', 'Type of Booking', 'Location Type',
            'Passenger(s)', 'No. of Passengers', 'Doc No/Req By',
        ...
        for row_idx, bk in enumerate(bookings, 2):
            values = [
                ...
                bk.employee_code or '',
                dict(bk._fields['booking_type'].selection).get(bk.booking_type, '') if bk.booking_type else '',
        ...
        wb.save(fp)
        fp.seek(0)
        self.write({'excel_file': base64.b64encode(fp.read()), 'file_name': 'package_tour_bookings.xlsx'})
        fp.close()
