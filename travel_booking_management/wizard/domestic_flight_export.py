...
class DomesticFlightExport(models.TransientModel):
    _name = 'travel.domestic.flight.booking.export'
    _description = 'Export Domestic Flight Bookings to Excel'
    ...
    def action_export(self):
        ...
        if self.env.context.get('active_ids') and self.env.context.get('active_model') == 'travel.domestic.flight.booking':
            domain = [('id', 'in', self.env.context['active_ids'])]
            if self.state_filter and self.state_filter != 'all':
                domain.append(('state', '=', self.state_filter))

        bookings = self.env['travel.domestic.flight.booking'].search(domain, order='booking_date asc')
        wb = openpyxl.Workbook()
        ws = wb.active
        ws.title = 'Domestic Flight Bookings'
        ...
        for row_idx, bk in enumerate(bookings, 2):
            values = [
                ...
                dict(bk._fields['airline'].selection).get(bk.airline, '') if bk.airline else '',
                bk.flight_number or '',
        ...
        wb.save(fp)
        fp.seek(0)
        self.write({'excel_file': base64.b64encode(fp.read()), 'file_name': 'domestic_flight_bookings.xlsx'})
        fp.close()
