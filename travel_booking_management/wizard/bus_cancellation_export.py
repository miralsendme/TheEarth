...
class BusCancellationExport(models.TransientModel):
    _name = 'travel.bus.cancellation.export'
    _description = 'Export Bus Cancellations to Excel'
    ...
    def action_export(self):
        ...
        if self.env.context.get('active_ids') and self.env.context.get('active_model') == 'travel.bus.cancellation':
            domain = [('id', 'in', self.env.context['active_ids'])]
            if self.state_filter and self.state_filter != 'all':
                domain.append(('state', '=', self.state_filter))

        records = self.env['travel.bus.cancellation'].search(domain, order='cancellation_date asc')
        wb = openpyxl.Workbook()
        ws = wb.active
        ws.title = 'Bus Cancellations'
        ...
        headers = [
            ...
            'PNR Number', 'Total Amount', 'Service Charge', 'Refund Amount',
        ...
        wb.save(fp)
        fp.seek(0)
        self.write({'excel_file': base64.b64encode(fp.read()), 'file_name': 'bus_cancellations.xlsx'})
        fp.close()
