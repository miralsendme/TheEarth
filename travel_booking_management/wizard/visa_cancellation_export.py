...
class VisaCancellationExport(models.TransientModel):
    _name = 'travel.visa.cancellation.export'
    _description = 'Export Visa Cancellations to Excel'
    ...
    def action_export(self):
        ...
        if self.env.context.get('active_ids') and self.env.context.get('active_model') == 'travel.visa.cancellation':
            domain = [('id', 'in', self.env.context['active_ids'])]
            if self.state_filter and self.state_filter != 'all':
                domain.append(('state', '=', self.state_filter))

        records = self.env['travel.visa.cancellation'].search(domain, order='cancellation_date asc')
        wb = openpyxl.Workbook()
        ws = wb.active
        ws.title = 'Visa Cancellations'
        ...
        headers = [
            ...
            'Employee Code', 'Type of Booking', 'Passenger Name', 'Passenger(s)',
            'Doc No/Req By', 'Reference Number', 'Description',
            'Total Fare', 'Refund Amount',
        ...
        for row_idx, rc in enumerate(records, 2):
            values = [
                ...
                rc.employee_code or '',
                dict(rc._fields['booking_type'].selection).get(rc.booking_type, '') if rc.booking_type else '',
                rc.passenger_name or '',
                (rc.passenger_names or '').replace('\n', ', '),
                rc.document_number or '',
                rc.reference_number or '',
                rc.description or '',
        ...
        wb.save(fp)
        fp.seek(0)
        self.write({'excel_file': base64.b64encode(fp.read()), 'file_name': 'visa_cancellations.xlsx'})
        fp.close()
