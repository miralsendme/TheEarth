...
class IntlFlightCancellationExport(models.TransientModel):
    _name = 'travel.intl.flight.cancellation.export'
    _description = 'Export International Flight Cancellations to Excel'
    ...
    def action_export(self):
        ...
        if self.env.context.get('active_ids') and self.env.context.get('active_model') == 'travel.intl.flight.cancellation':
            domain = [('id', 'in', self.env.context['active_ids'])]
            if self.state_filter and self.state_filter != 'all':
                domain.append(('state', '=', self.state_filter))

        records = self.env['travel.intl.flight.cancellation'].search(domain, order='cancellation_date asc')
        wb = openpyxl.Workbook()
        ws = wb.active
        ws.title = 'Intl Flight Cancellations'
        ...
        headers = [
            ...
            'Employee Code', 'Doc No/Req By', 'Trip Type',
            'Origin', 'Destination', 'Return Origin', 'Return Destination',
            'Travel Date (Onward)', 'Return Date', 'Airline',
            'Flight No (Onward)', 'Flight No (Return)', 'Class',
            'Passenger(s)', 'No. Cancelled', 'Ticket Number', 'PNR Number', 'Split PNR',
            'Fare', 'Service Charge', 'Refund Amount',
        ...
        for row_idx, rc in enumerate(records, 2):
            values = [
                ...
                dict(rc._fields['trip_type'].selection).get(rc.trip_type, '') if rc.trip_type else '',
                rc.origin_city or '',
                rc.destination_city or '',
                rc.return_origin or '',
                ...
                rc.num_cancelled_passengers or 0,
                rc.ticket_number or '',
                rc.pnr_number or '',
                rc.split_pnr or '',
                rc.fare or 0.0,
        ...
        wb.save(fp)
        fp.seek(0)
        self.write({'excel_file': base64.b64encode(fp.read()), 'file_name': 'intl_flight_cancellations.xlsx'})
        fp.close()
