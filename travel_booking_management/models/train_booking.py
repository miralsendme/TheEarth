...
class TrainBooking(models.Model):
    ...
    @api.onchange('passenger_names')
    def _onchange_passenger_names_upper(self):
    ...
    def _check_pnr_unique(self):
        for rec in self:
            ...
            if duplicate:
                raise ValidationError(
                    _('Duplicate PNR detected! The PNR "%(pnr)s" is already used '
                      'on booking %(ref)s. Please verify the PNR before saving.',
                      pnr=rec.pnr_number, ref=duplicate.name)
                )

    @api.depends('num_passengers', 'quota')
    def _compute_service_charge(self):
        for rec in self:
            sc_type = 'train_tatkal' if rec.quota == 'tatkal' else 'train_general'
            rec.service_charge = self.env['travel.service.charge'].get_service_charge(
                sc_type, num_pax=rec.num_passengers)
