...
class InternationalFlightBooking(models.Model):
    ...
    origin_country_id = fields.Many2one('res.country', string='From Country', required=True)
    destination_city = fields.Char(string='To City', required=True)
    destination_country_id = fields.Many2one('res.country', string='To Country', required=True)
    ...
    ], string='Trip Type', default='round_trip', required=True)
    ...
    visa_required = fields.Boolean(string='Visa Required', default=False)
    baggage_allowance = fields.Char(string='Baggage Allowance')
    layover_info = fields.Text(string='Layover / Transit Info')
